...
/**
 * Load the broadcast services installed by applications.
 *
 ...
 *    all services doesn't expose to us the intents a service has
 ...
 *
 * 2) Using PackageManager.queryBroadcastServices() to find all installed
 *    services works, but has numerous restrictions:
 *        * We need an internal list of actions that we support and
 *          query for.
 *        * Disabled components are never returned.
 *        * Services who's intent filters match certain data are only
 *          returned when our query matches the service's filters.
...
public class ActionReader {
    ...
    private static final boolean LOGV = false; //ListActivity.LOGV;
    private static final String TAG = "Background-Service" ; //Utils.TAG;
    ...
    private static enum ParserState { Unknown, InManifest,
        InApplication, InService, InIntentFilter, InAction }
     ...
     * Constructor.
     */
    public ActionReader(Context context) {
        mContext = context;
    ...
    public ArrayList<IntentFilterInfo> load() {
        ...
        List<android.content.pm.PackageInfo> packages =
                mPackageManager.getInstalledPackages(0x00000000);
        ...
        for (int i=0; i<packageCount; i++)
        {
            ...
            if (mOnLoadProgressListener != null) {
                ...
                // so that when a service status is toggled while we are
    ...
    void startService() {
        ...
        mCurrentState = ParserState.InService;
        ...
        if (componentName == null) {
            Log.e(TAG, "A service in "+mAndroidPackage.packageName+" has no name.");
        ...
        // until we are sure there are actually services in this package.
    ...
    void endService() {
        if (mCurrentState == ParserState.InService) {
        ...
        if (priorityRaw != null)
            try {
                ...
                Log.w(TAG, "Unable to parse priority value "+
                        "for service "+mCurrentComponent.componentName+
                        " in package "+mCurrentPackage.packageName+": "+priorityRaw);
            }
        if (LOGV && mCurrentFilterPriority != 0)
            Log.v(TAG, "Service "+mCurrentComponent.componentName+
    ...
    void startAction() {
        ...
        if (action == null) {
            Log.w(TAG, "Service "+mCurrentComponent.componentName+
                    " of package "+mCurrentPackage.packageName+" has "+
                    "action without name");
            return;
        }

        // Add this service to the result
    ...
    static boolean isSystemApp(android.content.pm.PackageInfo p) {
        ...
        // Of course, in those cases there'd be no services for us
    ...
    private String getAttr(String attributeName) {
        ...
        // In some, rarer cases (example: com.mxtech.videoplayer.MediaButtonService), the
